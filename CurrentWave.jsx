...
const CurrentWave = ({ today, todayKin, seals, tones, currentWaveOffset, setCurrentWaveOffset, waveData }) => {
  ...
  const DayCard = ({ day, index }) => {
    ...
    return (
      <div className={`bg-gray-800/50 backdrop-blur rounded-lg p-4 mb-3 border ${
        isToday ? 'border-purple-400 ring-2 ring-purple-400/50' : 'border-gray-700'
      }`}>
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex gap-1">
            {[1, 2, 3, 4, 5].map(level => (
              <button
                key={level}
                onClick={() => updateDayEnergy(day.date, level)}
                className={`w-6 h-6 rounded ${
                  day.energy === level ? energyColors[level] : 'bg-gray-700'
                } transition`}
              />
            ))}
          </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-purple-900 to-gray-900 text-white p-4 pb-20">
      ...
      <div className="max-w-2xl mx-auto mb-6 flex items-center justify-between">
        <button
          onClick={() => setCurrentWaveOffset(currentWaveOffset - 1)}
